...
function gate(ctx: Context): GateResult {
  ...
  if (chatType === 'group' || chatType === 'supergroup') {
    ...
    // Skip mention check for reactions — they can't contain mentions
    const isReaction = !!(ctx as any).messageReaction
    if (requireMention && !isReaction && !isMentioned(ctx, access.mentionPatterns)) {
      return { action: 'drop' }
    }
...
/** Extract quoted-reply context from a Telegram reply, if present. */
function replyPrefix(ctx: Context): string {
  const reply = ctx.message?.reply_to_message
  if (!reply) return ''
  const who = reply.from?.username ?? reply.from?.first_name ?? 'unknown'
  const quoted = reply.text ?? reply.caption ?? '(media)'
  // Truncate very long quotes to keep context concise
  const maxLen = 300
  const trimmed = quoted.length > maxLen ? quoted.slice(0, maxLen) + '…' : quoted
  return `[replying to ${who}: "${trimmed}"]\n`
}

bot.on('message:text', async ctx => {
  await handleInbound(ctx, replyPrefix(ctx) + ctx.message.text, undefined)
})
...
bot.on('message:photo', async ctx => {
  ...
  await handleInbound(ctx, replyPrefix(ctx) + caption, async () => {
...
bot.on('message:document', async ctx => {
  ...
  await handleInbound(ctx, replyPrefix(ctx) + text, undefined, {
    kind: 'document',
    file_id: doc.file_id,
    size: doc.file_size,
    mime: doc.mime_type,
...
bot.on('message:video_note', async ctx => {
  ...
  await handleInbound(ctx, replyPrefix(ctx) + '(video note)', undefined, {
    kind: 'video_note',
    file_id: vn.file_id,
    size: vn.file_size,
  })
...
bot.on('message:sticker', async ctx => {
  ...
  await handleInbound(ctx, replyPrefix(ctx) + `(sticker${emoji})`, undefined, {
    kind: 'sticker',
...
// Reaction handler — fires when someone reacts to any message in the chat.
// Requires 'message_reaction' in allowed_updates (see bot.start below).
bot.on('message_reaction', async ctx => {
  ...
  const from = update.user ?? ctx.from
  if (!from) return
  ...
  if (newEmojis.length === 0) return // reaction removed, not added

  const text = `(reacted with ${newEmojis.join(' ')})`
  mcp.notification({
    method: 'notifications/claude/channel',
    params: {
      content: text,
      meta: {
        ...
        user: ('username' in from && from.username) ? from.username : String(from.id),
        user_id: String(from.id),
        ts: new Date(update.date * 1000).toISOString(),
      },
    },
  }).catch(err => {
